...
BASE=`pwd`/..
PATCHES=$BASE/bering-uclibc-patches/2.4.34
WKS_SPACE=/tmp/k
...
tar jxf $BASE/kernels/linux-2.4.34.tar.bz2
...
#zcat $PATCHES/grsecurity-2.1.10-2.4.34-200701222213.patch.gz |  patch -p1
...
cat $PATCHES/linux-rtsp.patch |  patch -p1
...
bunzip2 $BASE/uml-patches/uml-2.4.34-bs3.patch.bz2 -c | patch -p1
...
cp $BASE/config/.config-2.4.34-bs3 .config
