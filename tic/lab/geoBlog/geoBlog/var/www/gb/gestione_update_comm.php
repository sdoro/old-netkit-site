...
while(list ($k, $valore) = each ($_POST)){
	...
	{	
		...
		{
			$stringa_sql="UPDATE NuoviCommenti SET daValutare=".$daValutare_new.", nonInseribile=".$nonInseribile_new." WHERE id=".$id;
...
{	
	...
	for($j=1;$j<count($arrayID);$j=$j+1)	
	{
		...
		$stringa="SELECT messaggio, quando, nickname, email, codValore, id_padre FROM NuoviCommenti WHERE id=".$arrayID[$j];
		...
		$strQueryIns="INSERT INTO Commenti (messaggio, quando, nickname, email, codValore, id_padre) VALUES ('";
		$strQueryIns=$strQueryIns.$msg."' , '";
		$strQueryIns=$strQueryIns.$linea["quando"]."' , '";
		$strQueryIns=$strQueryIns.$nick."' , '";
		$strQueryIns=$strQueryIns.$email."' , '";
		$strQueryIns=$strQueryIns.$linea["codValore"]."' , '";
		$strQueryIns=$strQueryIns.$linea["id_padre"]."')";		
		$nuovastringa_sql =mysql_query($strQueryIns);
		mysql_select_db("geoblog_admin") or die ("connessione fallita3");
		$strQueryDelete="DELETE FROM NuoviCommenti WHERE id=".$arrayID[$j];
